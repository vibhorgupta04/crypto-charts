...
const ExchangeCoin = () => {
  ...
  // handle error
  if (!loading && error) return <h2>Data could not be loaded, please try again.</h2>
  ...
  return (
    <form className="w-full rounded bg-white shadow px-6 py-4" onSubmit={handleExchangeCalculation}>
      ...
      <div className="py-4 flex flex-row items-center gap-6">
        ...
        <span>
          <select
            ...
            onChange={(e) => setSelectedExchange({ ...selectedExchange, sell: e.target.value })}
          >
            <option value="" disabled>Select Sell Exchange</option>
            {Object.values(exchangeList).map(({ name, unit, value }: ExchangeItem) =>
              <option
                className="px-4"
                key={unit + name}
                value={value + ' ' + unit}
        ...
        <span className="inline-flex items-center gap-2">
          ...
          />
          {calculationType === 'buy' && <span className="text-orange-400">{exchangeUnit}</span>}
        </span>
      </div>
      <div className="py-4 flex flex-row items-center gap-6">
        ...
        <span>
          <select
            ...
            onChange={(e) => setSelectedExchange({ ...selectedExchange, buy: e.target.value })}
          >
            <option value="" disabled>Select Sell Exchange</option>
            {Object.values(exchangeList).map(({ unit, name, value }: ExchangeItem) =>
              <option
                className="px-4"
                key={name}
                value={value + ' ' + unit}
        ...
        <span className="inline-flex items-center gap-2">
          ...
          />
          {calculationType === 'sell' && <span className="text-green-500">{exchangeUnit}</span>}
        </span>
      </div>
      <button className="bg-blue-600 text-white px-6 py-2 rounded my-4" type="submit">Exchange</button>
    </form>
  );
};
